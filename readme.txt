=== SIP calculator by Calculator.iO ===
Contributors: calculatorio
Tags: 
Requires at least: 5.0
Tested up to: 6.3.0
Stable tag: 1.0.0
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

Calculate SIP returns, growth & wealth accumulation with confidence using our SIP calculator. Make informed investment decisions.

== Description ==

Calculate SIP returns, growth & wealth accumulation with confidence using our SIP calculator. Make informed investment decisions.

Libraries in use:
1. https://mathjs.org/
2. https://katex.org/
3. https://github.com/aFarkas/lazysizes
4. https://github.com/RobinHerbots/Inputmask
5. https://air-datepicker.com/
6. https://www.chartjs.org/

== Installation ==

1. Upload the /ci_sip_calculator/ folder to the /wp-content/plugins/ directory.
2. Activate the [SIP calculator](https://www.calculator.io/sip-calculator/ "SIP calculator Homepage") plugin through the "Plugins" menu in WordPress.
3. Configure your settings via Appearance > Widgets > SIP calculator.
4. And you're good to go!

== Screenshots ==
1. The SIP calculator Input Form.
2. The SIP calculator Calculation Results.

== Changelog ==

= 1.0.0 =
* Initial release
